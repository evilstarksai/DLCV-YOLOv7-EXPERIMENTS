...
class BAM(nn.Module):
    def __init__(self, c1, c2=None, reduction_ratio=16, dilation=4):
        ...
        self.fc1 = nn.Linear(in_features=c1, out_features=self.hid_channel)
        self.bn1_1d = nn.BatchNorm1d(self.hid_channel)
        self.fc2 = nn.Linear(in_features=self.hid_channel, out_features=c1)
        self.bn2_1d = nn.BatchNorm1d(c1)

        self.conv1 = conv1x1(c1, self.hid_channel)
...
class CBAM(nn.Module):
    def __init__(self, c1, c2=None, reduction_ratio=16, dilation=1):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(in_features=c1, out_features=self.hid_channel),
            nn.ReLU(),
            nn.Linear(in_features=self.hid_channel, out_features=c1)
        )
